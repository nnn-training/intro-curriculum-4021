...
describe('/schedules/:scheduleId?delete=1', () => {
    ...
    test('予定に関連する全ての情報が削除できる', async() => {
        ...
        // 削除したScheduleIdに関わるレコードがCommentテーブル内に残留していない
        expect(comments.length).toBe(0);

        const availabilities = await Availability.findAll({
            where: { scheduleId: scheduleId }
        });
        // 削除したScheduleIdに関わるレコードがAvailavilityテーブル内に残留していない
        expect(availabilities.length).toBe(0);

        const candidates = await Candidate.findAll({
            where: { scheduleId: scheduleId }
        });
        // 削除したScheduleIdに関わるレコードがCandidateテーブル内に残留していない
        expect(candidates.length).toBe(0);

        const schedule = await Schedule.findByPk(scheduleId);
        // 削除したScheduleIdのレコードがScheduleテーブル内に残留していない
        // scheduleIdはScheduleテーブルのプライマリキーなので残っているはずがない
