...
describe('/schedules/:scheduleId/users/:userId/candidates/:candidateId', () => {
  ...
  test('出欠が更新できる', async () => {
    ...
    const res = await request(app).post('/schedules').send({ scheduleName: 'テスト出欠更新予定1', memo: 'テスト出欠更新メモ1', candidates: 'テスト出欠更新候補1' });
    const createdSchedulePath = res.headers.location;
    scheduleId = createdSchedulePath.split('/schedules/')[1];
...
describe('/schedules/:scheduleId/users/:userId/comments', () => {
  ...
  test('コメントが更新できる', async () => {
    ...
    scheduleId = createdSchedulePath.split('/schedules/')[1];
    // 更新がされることをテスト
    await request(app).post(`/schedules/${scheduleId}/users/${userId}/comments`).send({ comment: 'testcomment' }).expect('{"status":"OK","comment":"testcomment"}');
...
describe('/schedules/:scheduleId/update', () => {
  ...
  test('予定が更新でき、候補が追加できる', async () => {
    ...
    const res = await request(app).post('/schedules').send({ scheduleName: 'テスト更新予定1', memo: 'テスト更新メモ1', candidates: 'テスト更新候補1' });
    const createdSchedulePath = res.headers.location;
    scheduleId = createdSchedulePath.split('/schedules/')[1];
    // 更新がされることをテスト
    await request(app).post(`/schedules/${scheduleId}/update`).send({ scheduleName: 'テスト更新予定2', memo: 'テスト更新メモ2', candidates: 'テスト更新候補2' });
...
describe('/schedules/:scheduleId/delete', () => {
  ...
  test('予定に関連する全ての情報が削除できる', async () => {
    ...
    const res = await request(app).post('/schedules').send({ scheduleName: 'テスト削除予定1', memo: 'テスト削除メモ1', candidates: 'テスト削除候補1' });
    const createdSchedulePath = res.headers.location;
    const scheduleId = createdSchedulePath.split('/schedules/')[1];

    // 出欠作成
    const candidate = await prisma.candidate.findFirst({ where: { scheduleId } });
    await request(app).post(`/schedules/${scheduleId}/users/${userId}/candidates/${candidate.candidateId}`).send({ availability: 2 }); // 出席に更新
